...
- [x] 設定画面・アップグレード画面から、2プランそれぞれの申し込みボタンでStripeのチェックアウト画面まで遷移できる（2026-09-12確認済み。※実際の決済完了までは、返金対応の手間を避けるため未実施）
- [x] HACCP側から`/api/items`が正常にデータを取得できる（接続テストボタンで商品26件を確認・実際の登録件数と一致。2026-09-12確認済み）

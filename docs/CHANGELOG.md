...
## 2026-09-12 - 仕入先マスタ・発注書機能の追加

### 追加・変更
- 新しいページ `/suppliers`（ナビゲーションに「仕入先」として常時表示）を新設。仕入先名・メールアドレス・電話番号・住所・メモを登録・編集・削除できる。新規Firestoreコレクション`suppliers`を使用。
- 各仕入先の詳細から「発注書」ボタンで `/suppliers/[id]/order` に遷移し、その仕入先を「主な仕入先」に指定している商品を一覧表示。発注点（`minStock`）を下回っている商品はあらかじめチェック・推奨数量が入った状態で表示され、数量は自由に編集可能。
- 発注書は「印刷 / PDF保存」（ブラウザの印刷機能。ラベル印刷と同じ方式で新規ライブラリは追加していない）と、「仕入先にメール送信」（新規API `POST /api/purchase-order/send`、既存のResend設定を流用）の2通りで出力できる。
- 商品登録・編集フォームの「主な仕入先」欄（HACCP連携項目ON時のみ表示）に、`/suppliers`に登録済みの名前を入力候補として出す機能を追加（表記ゆれ防止。自由入力自体は従来通り可能）。
- 毎日の自動バックアップメール（`/api/cron/backup`）のバックアップ対象に`suppliers`を追加。

### セキュリティ
- `POST /api/purchase-order/send` は、他の外部送信系APIと同じくFirebase IDトークンで本人確認したうえで、操作対象のsupplierドキュメントの`userId`が本人と一致することもサーバー側で確認してから送信する（他人の仕入先に成り済まして勝手にメールを送れないようにするため）。

### デプロイ時の注意
**[要対応・ユーザー側の作業]** 新規コレクション`suppliers`を使うため、`docs/firestore.rules_案.txt`に`suppliers`用のルールを追記した。**この変更をデプロイしたら、Firebaseコンソール→「Firestore Database」→「ルール」タブで、更新後の`firestore.rules_案.txt`の内容を貼り付けて公開しないと、`/suppliers`ページが権限エラー（permission-denied）で動かない。**

### 動作確認チェックリスト
- [ ] Firestoreルールを再公開した後、`/suppliers`で仕入先を新規登録・編集・削除できる
- [ ] 商品登録・編集画面の「主な仕入先」欄に、登録した仕入先名が入力候補として出る
- [ ] 発注点を下回っている商品が、対応する仕入先の発注書ページで自動的にチェック済みで表示される
- [ ] 発注書ページの「印刷 / PDF保存」で、見やすい形で印刷プレビューが出る
- [ ] 仕入先にメールアドレスを登録した状態で「仕入先にメール送信」を押すと、実際にそのメールアドレス宛に発注内容が届く
- [ ] 仕入先にメールアドレスが未登録の場合、送信しようとすると分かりやすいエラーが出る

---

## 2026-09-12 - 賞味期限アラート機能の追加

### 追加・変更
- 新しいページ `/expiry`（ナビゲーションに「期限管理」として追加、設定で「HACCP連携項目を表示」がONのときのみ表示）を新設。入庫時に記録した賞味期限（`bestBefore`）と現在の在庫数から、先入先出（FIFO）の考え方で「今、期限が近いと推定される在庫」を一覧表示する。
- 表示期間は3日／7日／14日／30日から選択可能。期限切れ・3日以内・7日以内で色分け表示。
- 新規ファイル `src/lib/expiry.ts`：`lib/valuation.ts`の`calculateFIFO`と同じ考え方で「現在庫として残っていると推定されるロット」を算出するロジックを追加。

### 注意点（重要）
- このアプリは「どのロットが何個残っているか」を個別には記録しておらず、商品ごとの合計在庫数と入庫履歴だけを持っている。そのため賞味期限アラートはあくまで**推定**であり、実際の出庫が先入先出の順番通りに行われていなかった場合はズレることがある。画面内にもその旨の注意書きを表示している。
- メールでの自動通知（毎日のバックアップメールのような形）は今回は含めていない。まずは画面での確認を先に導入し、動作確認後に追加を検討する。

### 動作確認チェックリスト
- [ ] 設定画面で「HACCP連携項目を表示」をONにすると、ナビゲーションに「期限管理」リンクが表示される
- [ ] 商品の入庫時に賞味期限を入力し、その商品の現在庫数が0より多い状態で、`/expiry`にその商品が正しい期限・数量で表示される
- [ ] 期限が「7日以内」等、選択した期間より先の商品は一覧に表示されない
- [ ] 期限切れ（今日より前）の商品が「期限切れ」の赤いバッジ付きで表示される
- [ ] 一覧から商品名をクリックすると、その商品の詳細ページに遷移する
- [ ] 「HACCP連携項目を表示」がOFFの場合、`/expiry`ページ上部に設定画面への案内が表示される

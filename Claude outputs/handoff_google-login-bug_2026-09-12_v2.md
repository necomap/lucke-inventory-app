...
**【2026-09-12 追記・解決済み】** この文書の作成後、`AuthContext.tsx`のコミット・pushが完了し、本番環境で「毎回アカウント選択画面が表示される」ことが確認できた。**Googleログイン関連の不具合は、現時点ではすべて解決している。** 以下の「ファイルが勝手に元に戻る」現象自体の原因（OneDrive・他のクラウド同期・VS Code等のエディタ・バックグラウンドのAIツール、いずれも本人に確認済みで否定）は、**結局特定できないまま**だった。今後また似た現象（Claudeが書き込んだ内容が、パソコン上でしばらくすると元に戻る）が起きた場合の参考記録として、この文書はそのまま残しておく。そのときの対処法は「原因を追わず、正しい内容になった瞬間にすぐGit commit・pushして確定させる」のが実用的だった。
...
### 重要な補足（ユーザー確認済み）
ユーザーは「ファイルの内容はこちらでは一切触っていないし開いてもいない」と明言している。つまり、ユーザーが手動で元に戻した・GitHub Desktopで誤って「Discard changes」した、といった**人為的操作ではない**。何らかの自動的な仕組み（同期ソフト、バックアップソフト、セキュリティソフトの復元機能など）がバックグラウンドで書き換えている可能性が高い。

### 疑われる原因（検証結果あり）
- **OneDriveは使用していないことをユーザーに確認済み（否定）。**
- **OneDrive以外のクラウド同期サービス（Dropbox・Google Drive・iCloud・会社のバックアップソフト等）も使用していないことをユーザーに確認済み（否定）。**
- **未確認（次のチャットで確認すべき最有力候補）:** このプロジェクトフォルダ(`inventory-app`)が、VS CodeやCursorなど何かのコードエディタで**開きっぱなし**になっていて、`AuthContext.tsx`（や`CHANGELOG.md`・`login/page.tsx`）のタブがその中で開いたままになっている可能性。もしそのエディタで「自動保存」が有効になっていると、Claudeが外部から書き込んだ新しい内容を、エディタが持っている（触っていない・保存していない）古いバッファの内容で自動保存時に上書きしてしまい、結果的に「元に戻った」ように見える、という現象が起こり得る。この仮説はユーザーにまだ確認できていない（「開いていない」という返答はあったが、それが「今回新しく開いた覚えはない」という意味なのか、「そもそも一切開いていない（既存のタブも含めて）」という意味なのか、明確に切り分けられていない）。
  - 確認方法: タスクバーやタスクマネージャーで、VS Code・Cursor・その他エディタが起動していないか確認する。起動していれば、`inventory-app`フォルダが開かれていないか、`AuthContext.tsx`等のタブが残っていないか確認し、あれば一度そのエディタを完全に閉じてから、Claude側で再度ファイルを書き込み、少し時間を置いてから内容が保持されているか確認する。
- その他の可能性として、GitHub Desktopの操作（過去に「secret detected」の対応でコミットを複数回Undoした際、意図した範囲より巻き戻ってしまった）も一因として疑ったが、今回の`AuthContext.tsx`の巻き戻りは、Push・デプロイの話をする**前の時点、Claudeが書き込んだ直後**に既に起きている可能性があり、Git操作だけでは説明がつかない。
- まだ検証していないその他の可能性: Windows Defenderの「コントロールされたフォルダーアクセス（ランサムウェア対策）」やその他セキュリティソフトによる復元機能、何らかのファイル監視・自動生成スクリプト（`npm run dev`等の開発サーバーが動いたままになっていないかも確認する価値がある）。

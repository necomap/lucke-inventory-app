...
### HACCPアプリ（外部）との検品自動連動
Luckeが提供する外部の「HACCP温度・衛生管理アプリ（ `https://haccp.lucke.jp/` ）」と完全に連動しています。

- **動作**:  
  HACCPアプリ側で食材の「納品検品記録（温度測定など）」を行うと、**Lucke Inventory側の在庫数に自動でその食材の数量が加算され、入庫履歴も自動的に記録されます。**
- **メリット**:  
  検品時に衛生アプリに記録を付けるだけで在庫登録が完了するため、Lucke側で二重に入力する手間がなくなります。
- **詳細確認**:  
  システム設定（`/settings`）の「HACCP連携項目を表示」をONにすると、設定画面内にHACCPアプリへの直接のリンクボタンが表示されます。
